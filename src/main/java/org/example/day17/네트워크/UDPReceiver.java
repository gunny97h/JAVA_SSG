...
public class UDPReceiver {
    public static void main(String[] args) throws IOException {
        // 수신할 공간을 다 만들어두고 받을 준비를 먼저 하고 있어야 함.
        ...
        // Stream 전송시 1 byte 단위로 읽고 쓴다.
        // --> byte[] 배열로 데이터를 설정하면 됨.
        byte[] data = new byte[256];

        // 보낼 때는 DatagramPacket으로 보냄.
        // 받을 때도 DatagramPacket으로 받을 수 있게 미리 객체를 만들어둠!
        DatagramPacket packet = new DatagramPacket(data, data.length);
        // 패킷으로 받으면 데이터(데이터그램)를 꺼내서 위에서 설정한 data배열에 넣어줌.

        System.out.println("데이터 전송되기를 기다리는 중... ");
        socket.receive(packet);
        // 현재까지 받은 데이터는 어디에 들어있을까?
        // --> byte[]에 있음.
